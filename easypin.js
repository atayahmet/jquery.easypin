(function($){

	$.fn.easypin = function(options) {

		options = options || {};

		var parentClass = $.fn.easypin.defaults.parentClass;
		var pinMapClass = $.fn.easypin.defaults.pinMapClass;
		var hoverClass = $.fn.easypin.defaults.hoverClass;
		var dashWidth = $.fn.easypin.defaults.dashWidth;
		var imageZindex = $.fn.easypin.defaults.imageZindex;
		var pinMapZindex = $.fn.easypin.defaults.pinMapZindex;
		var hoverLayerZindex = $.fn.easypin.defaults.hoverLayerZindex;

		// set default options values and became user side
		$.extend($.fn.easypin.defaults, options );

		this.each(function(i) {

			var containerElement = $(this)
				.after($('<div/>', {'class': parentClass}))
				.appendTo(setClass(parentClass)+':last')
				.css('position', 'absolute')
				.css('z-index', imageZindex);


			$(this).addClass('easypin-target');

			var imageWidth = $(this).width();
			var imageHeight = $(this).height();

			containerElement
				.parent()
				.attr($.fn.easypin.config('widthAttribute'), imageWidth)
				.attr($.fn.easypin.config('heightAttribute'), imageHeight)
				.css({
					width: setPx(imageWidth),
					height: setPx(imageHeight),
					position: $.fn.easypin.config('parentPosition'),
					border: setPx(dashWidth)+' dashed #383838'
				});

		});

		// hover event
		var parentElement = $(setClass(parentClass));

		$(parentElement).hover(function() {

			$(this)
				.prepend(
					$('<div/>', {'class': hoverClass})
						.css({
							width: '100%',
							height: '100%',
							position: 'absolute',
							opacity: 0,
							'z-index': hoverLayerZindex,
							'background-color': 'black',
							cursor: 'copy'
						})
				)
				// cross to children object
				.children(setClass(hoverClass))
				.animate({
				    opacity: 0.4,
				}, 800);

		},function() {
			$(this)
				// cross to children object
				.children(setClass(hoverClass))
				.animate({
				    opacity: 0,
				}, 800, function() {
			   		$(this).remove();
			});	

		})
		.append($('<div/>', {'class': pinMapClass})
			// child element
			.css({
				position: 'absolute',
				'z-index': pinMapZindex
			})
		)
		.bind('mousedown', function(e) {
			
			if(e.which != 1) return;

			var parentElement = e.currentTarget;
			var imageWidth = $('img'+setClass('easypin-target'), parentElement).width();
			var imageHeight = $('img'+setClass('easypin-target'), parentElement).height();

			// pin map class sized
			$(setClass(pinMapClass), parentElement).css({
				width: setPx(imageWidth),
				height: setPx(imageHeight)
			});

			var src = $.fn.easypin.defaults.markerSrc;
			var markerWidth = $.fn.easypin.defaults.markerWidth;
			var markerHeight = $.fn.easypin.defaults.markerHeight == 'auto' ? markerWidth : $.fn.easypin.defaults.markerHeight;
			var markerClass = $.fn.easypin.defaults.markerClass;
			var markerContainerZindex = $.fn.easypin.defaults.markerContainerZindex;

			var dashWidth = $.fn.easypin.defaults.dashWidth;
			var posYBalance = $.fn.easypin.defaults.posYBalance;
			var posXBalance = $.fn.easypin.defaults.posXBalance;

			var targetImage = $('img'+setClass('easypin-target'), parentElement);
			var imagePositionY = targetImage.offset().top - (dashWidth-posYBalance);
			var imagePositionX = targetImage.offset().left - (dashWidth-posXBalance);
			var clickPosX = (e.pageX-imagePositionX);
			var clickPosY = (e.pageY-imagePositionY);

			var markerBorderX = clickPosX-(markerWidth/2);
			var markerBorderY = clickPosY-(markerHeight/2);

			if(markerBorderX < 0) {
				markerBorderX = 0;
			}
			else if(clickPosX+(markerWidth/2) > imageWidth) {
				markerBorderX = imageWidth-markerWidth;
			}

			if(markerBorderY < 0) {
				markerBorderY = 0;
			}
			else if(clickPosY+(markerHeight/2) > imageHeight) {
				markerBorderY = imageHeight-markerHeight;
			}

			// create marker container
			var markerContainer = $('<div/>', {'class': markerClass})
				.css('left', setPx(markerBorderX))
				.css('top', setPx(markerBorderY-5))
				.css('width', markerWidth)
				.css('height', markerHeight)
				.css('position', 'absolute')
				.css('opacity', 0)
				.css('z-index', markerContainerZindex+10)
				.css('background-image', 'url('+src+')')
				.css('background-size', setPx(markerWidth))
				.css('cursor', 'move');

			// marker image append to marker container
			//$(markerContainer).append(markerImage);

			// marker container append to pin parent container
			$(parentElement).append(markerContainer);

			// marker animate
			$(markerContainer).animate({opacity: 1, top: setPx(markerBorderY)});


			var draggable = $(".easy-marker");                    // getting all divs with drag class name

	        // binding methods mousedown and mousemove
	        $(draggable).bind('mousedown', function (e) {

	        	e.stopPropagation();

				if(e.which != 1) return;

	            $(draggable).bind('mousemove', function (e) {

	            	var parentElement = $(e.currentTarget).parent();
	            	var markerContainer = $(e.currentTarget);
	            	var targetImage = $('img.easypin-target', parentElement)
	            	
	            	liveY = e.pageY-targetImage.offset().top;
	            	liveX = e.pageX-targetImage.offset().left;

	            	var relY = liveY;
	            	var relX = liveX;

	            	if(liveY - (markerHeight/2) < 0) {
	            		var relY = (markerHeight/2);
	            	}
	            	else if(liveY + (markerHeight/2) > imageHeight) {
	            		var relY = imageHeight-(markerHeight/2);
	            	}

	            	if(liveX - (markerWidth/2) < 0) {
	            		var relX = (markerWidth/2);
	            	}
	            	else if(liveX + (markerWidth/2) > imageWidth) {
	            		var relX = imageWidth-(markerWidth/2);
	            	}

	                $(markerContainer).css({
	                	position: 'absolute', 
	                	top: setPx(relY),
	                	left: setPx(relX),
	                	marginTop: -(markerHeight/2), 
	                	marginLeft: -(markerWidth/2)
	               	});


	            });
	        });

	        // unbinding the events and removing  
	        $(draggable).bind('mouseup', function () {
	            $(draggable).unbind('mousemove');
	        });
		});

		return this;
	};

	$.fn.easypin.config = function(attr) {
		return $.fn.easypin.defaults[attr];
	};

	$.fn.easypin.di = function(key, depends) {

		var index = $.fn.easypin.container['$index'];

		if(! $.fn.easypin.container[index]) {
			$.fn.easypin.container[index] = {};
		}

		$.fn.easypin.container[index][key] = depends;
	};

	$.fn.easypin.diMarker = function(key, depends, index) {

		var parentIndex = $.fn.easypin.container['$index'];

		if(! $.fn.easypin.markerContainer[parentIndex]) {
			$.fn.easypin.markerContainer[parentIndex] = {};
		}

		if(! $.fn.easypin.markerContainer[parentIndex][key]) {
			$.fn.easypin.markerContainer[parentIndex][key] = {};
		}

		$.fn.easypin.markerContainer[parentIndex][key][index] = depends;
	};

	$.fn.easypin.call = function(func, params) {

		params = params || '';

		var depends = func.toString().match(/function\s*\(\s*(.*?)\s*\)/i);

		if(depends.length > 1) {

			depends = depends[1];

			var clientParm = depends.replace(/(\$[a-zA-Z]+)/g, '');
			clientParm = clientParm.replace(/\s+/g, '');
			clientParm = clientParm.replace(/,+/g,',');
			clientParm = clientParm.replace(/(^,)/, '');
			clientParm = clientParm.split(/,/g);
			
			expectParm = depends.match(/(\$[a-zA-Z]+)/g);
 
			var dependsArgs = new Array();
			var index = getIndex();

			for(var i in expectParm) {
				if($.fn.easypin.container[index][expectParm[i]]) {
					dependsArgs.push($.fn.easypin.container[index][expectParm[i]]);
				}
			}

			dependsArgs.push(params);
			func.apply(null, dependsArgs);
		}
	};

	$.fn.easypin.active = {

		mousedown: true

	};

	$.fn.easypin.defaults = {

		widthAttribute: 'data-width',
       	heightAttribute: 'data-height',
       	xAttribute: 'data-x',
       	yAttribute: 'data-y',
       	parentPosition: 'relative',
       	parentClass: 'pinParent',
		pinMapClass: 'pinCanvas',
		hoverClass: 'hoverClass',
		markerClass: 'easy-marker',
		markerSrc: 'data:image/png;base64,',
		markerWidth: 32,
		markerHeight: 'auto',
		animate: false,
		posYBalance: 2,
		posXBalance: 2,
		dashWidth: 2,
		imageZindex: 1,
		pinMapZindex: 2,
		hoverLayerZindex: 3,
		markerContainerZindex: 4,
		markerBorderColor: '#FFFF00',
		downPoint: 10
	
	};

	$.fn.easypin.container = {};
	$.fn.easypin.markerContainer = {};

	var setClass = function(name) {
		return '.'+name;
	};

	var setPx = function(num) {
		return num+'px';
	};

	var setIndex = function(index) {
		$.fn.easypin.container['$index'] = index;
	};

	var getIndex = function() {
		return $.fn.easypin.container['$index'];
	};

	var getMarkerLastIndex = function() {
		return $.fn.easypin.container[getIndex()]['$markerLastIndex'] || 0;
	};

	var setMarkerLastIndex = function(index) {
		$.fn.easypin.container[getIndex()]['$markerLastIndex'] = index;
	};

	var calcElementPagePos = function($imageObj, e) {

		var dashWidth = $.fn.easypin.defaults.dashWidth;
		var posYBalance = $.fn.easypin.defaults.posYBalance;
		var posXBalance = $.fn.easypin.defaults.posXBalance;

		$.fn.easypin.di('$imagePositionY', $imageObj.offset().top - (dashWidth-posYBalance));
		$.fn.easypin.di('$imagePositionX', $imageObj.offset().left - (dashWidth-posXBalance));
		
	};

	var calcClickPosInCanvas = function($imagePositionY, $imagePositionX, $pageY, $pageX, e) {

		$.fn.easypin.di('$clickPosX', ($pageX-$imagePositionX));
		$.fn.easypin.di('$clickPosY', ($pageY-$imagePositionY));

	};

	var createMarker = function($containerElement, $clickPosX, $clickPosY, $imageWidth, $imageHeight, $clickPosX, e) {

		var e = e.e;
		var index = e.index;
		var src = $.fn.easypin.defaults.markerSrc;
		var className = $.fn.easypin.defaults.markerClass;
		var markerWidth = $.fn.easypin.defaults.markerWidth;
		var markerHeight = $.fn.easypin.defaults.markerHeight;
		var markerClass = $.fn.easypin.defaults.markerClass;
		var markerContainerZindex = $.fn.easypin.defaults.markerContainerZindex;
		var animate = $.fn.easypin.defaults.animate;
		var downPoint = animate ? $.fn.easypin.defaults.downPoint : 0;
		var markerWidth = $.fn.easypin.defaults.markerWidth;
		var currentMarkerIndex = getMarkerLastIndex()+1;
		setMarkerLastIndex(currentMarkerIndex);

		var markerImage = $('<img/>')
			.attr('src', src)
			.css('width', markerWidth)
			.css('height', markerHeight);

		var markerHeight = 64;
		var markerBorderX = Math.floor($clickPosX-(markerWidth/2));
		var markerBorderY = Math.floor($clickPosY-(markerHeight/2));

		if(markerBorderX < 0) {
			markerBorderX = 0;
		}
		else if($clickPosX+(markerWidth/2) > $imageWidth) {
			markerBorderX = Math.floor($imageWidth-markerWidth);
		}

		if(markerBorderY < 0) {
			markerBorderY = 0;
		}
		else if($clickPosY+(markerHeight/2) > $imageHeight) {
			markerBorderY = Math.floor($imageHeight-markerHeight);
		}

		var markerContainer = $('<div/>', {'class': className})
			.attr('data-index', currentMarkerIndex)
			.attr('id', 'marker_'+getIndex()+'_'+currentMarkerIndex)
			.attr('draggable', 'true')
			.css('left', setPx(markerBorderX))
			.css('top', setPx(markerBorderY-5))
			.css('width', markerWidth)
			.css('height', markerHeight)
			.css('position', 'absolute')
			//.css('opacity', 0)
			.css('z-index', markerContainerZindex);
			//.css('cursor', 'move');

		markerContainer.append(markerImage);

		$containerElement.append(markerContainer);

		/*
		$(markerContainer).unbind('mousemove');



		markerContainer
		.bind('mousedown', mouseDownAction)
		.bind('mouseup', mouseUpAction);

		// set dependencies
		$.fn.easypin.di('$currentMarkerContainerIndex', currentMarkerIndex);
		$.fn.easypin.di('$currentMarkerContainer', markerContainer);

		// set marker dependencies
		$.fn.easypin.diMarker('$markerContainer', markerContainer, currentMarkerIndex);

		if(animate) {
			$.fn.easypin.call(setAnimate, {markerContainer: markerContainer, clickPosY: markerBorderY});
		}else{
			markerContainer.css('opacity', 1);
		}*/

	};

	var mouseDownAction = function(e) {

		if(e.which != 1) {
			return;
		}

		$.fn.easypin.active.mousedown = false;

		var markerInex = $(e.currentTarget).attr('data-index');
		var markerContainer = $.fn.easypin.markerContainer[getIndex()]['$markerContainer'][markerInex];	
		var containerElement = $.fn.easypin.container[getIndex()]['$containerElement'];

		markerContainer.bind('mousemove', moveAction);
		
		$.fn.easypin.di('$containerElement', containerElement);
		$.fn.easypin.di('$currentMarkerContainer', markerContainer);

	};

	var moveAction = function(e) {
		
		var markerContainer = $.fn.easypin.container[getIndex()]['$currentMarkerContainer'];

		// set current positions
		$.fn.easypin.di('$pageY', e.pageY);
		$.fn.easypin.di('$pageX', e.pageX);
		$.fn.easypin.call(calcElementPagePos, e);
		$.fn.easypin.call(calcClickPosInCanvas, markerContainer);

		$.fn.easypin.call(moveTo, e);

	};

	var mouseUpAction = function(e) {

		//console.log('x');
		var markerContainer = $.fn.easypin.container[getIndex()]['$currentMarkerContainer'];
		var containerElement = $.fn.easypin.container[getIndex()]['$containerElement'];

		containerElement.unbind('mousemove');

	};

	var moveTo = function($clickPosX, $clickPosY, markerContainer) {

		var markerContainer = $.fn.easypin.container[getIndex()]['$currentMarkerContainer'];
		markerContainer.css({
			left: setPx($clickPosX-16),
			top: setPx($clickPosY)
		});

		//console.log($clickPosX+' ' +$clickPosY);


	};
	var getMarkerUrl = function() {
		return $.fn.easypin.defaults.markerSrc;
	};

	var setAnimate = function(params) {

		var parentIndex = getIndex();
		var markerIndex = getMarkerLastIndex();
		var currentElem = params.markerContainer;
		var prevPosY = params.clickPosY;

		currentElem.animate({
		    opacity: 1,
		    top: setPx(prevPosY)
		}, '100');

		var params = {
    		markerContainer: currentElem,
    		clickPosY: prevPosY
    	};

		currentElem.animate(
	        {
	        	'top': setPx(prevPosY-3)
	        }, 
	        { 
		        duration: 700, 
		        complete: function() {
		        	currentElem.animate({
		        		'top': setPx(prevPosY)
		        	})

		        	$.fn.easypin.call(setAnimate, params);
	            }
	        }
	    ).hover(function(){
	    	$(this).stop(true, true);

	    	$(this)
	    		.css({
	    			outline: '1px dashed '+$.fn.easypin.defaults.markerBorderColor,
	    			padding: '2px'	
	    		});
	    },function() {
	    	$(this).css('outline', 'inherit');
	    	$.fn.easypin.call(setAnimate, params);
	    });
	};

}(jQuery));



	
